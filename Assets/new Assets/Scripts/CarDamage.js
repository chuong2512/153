...
function OnCollisionEnter (collision : Collision)
	...
	if (collision.relativeVelocity.magnitude >= minForce)
		...
		if (DeformColliders.length > 0)
			...
			for (var k=0; k<DeformColliders.length; k++)
				{
				// Necesario un mesh intermedio, no sirve mandar sharedMesh a deformar
				
				var mesh : Mesh = new Mesh();
				mesh.vertices = DeformColliders[k].sharedMesh.vertices;
				mesh.triangles = DeformColliders[k].sharedMesh.triangles;
				
				DeformMesh(mesh, m_colliderData[k].permaVerts, DeformColliders[k].transform, collision.contacts, contactForce);				
				DeformColliders[k].sharedMesh = mesh;
				}
		...
		for (var g=0; g<DeformNodes.length; g++)
			m_nodeModified[g] = false;
		...
		for (var c=0; c<collision.contacts.length; c++)
			...
			for (g=0; g<DeformNodes.length; g++)
				if (!m_nodeModified[g]) m_nodeModified[g] = DeformNode(DeformNodes[g], m_permaNodes[g], m_permaNodeAngles[g], contactPoint, contactForce);
			}
		}
	}
